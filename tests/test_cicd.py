"""Tests for agentic_uv.cicd module."""
...
from agentic_uv.cicd.fetchers import GitHubRepo, get_github_release, get_github_tag, get_pypi_version
from agentic_uv.cicd.updaters import ActionYmlUpdater, PreCommitConfigUpdater, PyprojectTomlUpdater
...
class TestFetchers:
    def test_get_pypi_version_returns_version(self) -> None:
        ...
        with patch("agentic_uv.cicd.fetchers._fetch_json", return_value=mock_response):
            assert get_pypi_version("pytest") == "1.2.3"

    def test_get_pypi_version_returns_none_on_failure(self) -> None:
        with patch("agentic_uv.cicd.fetchers._fetch_json", return_value=None):
...
class TestPyprojectTomlUpdater:
    def test_updates_package_version(self, temp_pyproject: Path) -> None:
        with (
            patch("agentic_uv.cicd.updaters.PYPI_PACKAGES", ["pytest"]),
            patch("agentic_uv.cicd.updaters.get_pypi_version", return_value="8.0.0"),
        ):
    ...
    def test_updates_package_with_extras(self, temp_pyproject: Path) -> None:
        with (
            patch("agentic_uv.cicd.updaters.PYPI_PACKAGES", ["mkdocstrings"]),
            patch("agentic_uv.cicd.updaters.get_pypi_version", return_value="0.30.0"),
        ):
            count = PyprojectTomlUpdater([temp_pyproject]).update()
...
class TestActionYmlUpdater:
    def test_updates_uv_version(self, temp_action_yml: Path) -> None:
        with patch("agentic_uv.cicd.updaters.get_github_release", return_value="0.9.7"):
...
class TestPreCommitConfigUpdater:
    def test_updates_hook_revision(self, temp_precommit: Path) -> None:
        ...
        with (
            patch("agentic_uv.cicd.updaters.PRECOMMIT_HOOKS", hooks),
            patch("agentic_uv.cicd.updaters.get_github_tag", return_value="5.0.0"),
        ):
