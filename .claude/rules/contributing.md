...
3. **Generated Projects Must**
   - Have `.claude/` directory with all 5 rule files
...
make test       # Must pass
make claude-info  # Shows Claude integration
ls -la .claude/  # Verify all 5 rule files exist
```
...
4. Document in generated `.claude/CLAUDE.md` if user-facing

### Updating `.claude/` Guidelines

The template includes `.claude/CLAUDE.md` and `.claude/rules/*.md` for generated projects.

**When updating these files:**

1. Ensure all Jinja2 syntax is correct (author name, choices, paths)
2. Include example code that follows the guidelines
3. Reference other rule files with `@.claude/rules/filename.md`
...
See also: @.claude/rules/best-practices.md
```
...
5. Update `.claude/rules/best-practices.md` if major change
...
❌ **Complex conditionals in `.claude/` markdown:**
```markdown
## Linting
...
- Generated project guide: `.claude/CLAUDE.md` (in template)
