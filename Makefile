...
bake-and-test-deploy: ## For quick publishing to agentic-uv-example to test GH Actions
	@rm -rf agentic-uv-example || true
	@uv run cookiecutter --no-input . --overwrite-if-exists \
		author="Florian Maas" \
		email="fpgmaas@gmail.com" \
		github_author_handle=fpgmaas \
		project_name=agentic-uv-example \
		project_slug=agentic_uv_example
	@cd agentic-uv-example; uv sync && \
		git init -b main && \
		git add . && \
		...
		git remote add origin git@github.com:fpgmaas/agentic-uv-example.git && \
